...
import { Button, SearchField, Spinner, Switch } from '@heroui/react';
import {
  LuCable,
  LuChevronRight,
  LuMapPin,
      ...
      {collapsed && !isMobile ? (
        <div className="flex flex-1 flex-col items-center gap-2 overflow-y-auto p-2">
          <Button
            isIconOnly
            size="sm"
            variant={layers.fat ? 'secondary' : 'ghost'}
            aria-label={`${layers.fat ? 'Ocultar' : 'Mostrar'} CTO (FAT)`}
            onPress={() => onLayerChange('fat', !layers.fat)}
          >
            <LuRadioTower className="size-4" />
          </Button>
          <Button
            isIconOnly
            size="sm"
            variant={layers.cables ? 'secondary' : 'ghost'}
            aria-label={`${layers.cables ? 'Ocultar' : 'Mostrar'} Cabos`}
            onPress={() => onLayerChange('cables', !layers.cables)}
          >
