...
Este documento descreve o **domínio operacional** do GigaCenter: entidades, status, fluxos e regras que a UI e a API aplicam. É a peça mais importante para o futuro GigaHub.
...
| `isLiveLocationActive` | Se o técnico está enviando localização contínua |
...
- Exige políticas de campo (localização fresca, sem bloqueios operacionais)
...
Configurável via UI interna (`/os/assuntos`).
...
| Bloqueios operacionais | Funcionário não pode estar bloqueado para ações de campo |
| Geofence na finalização | ≤ **300 m** do ponto da OS |
| Localização contínua | UI de campo incentiva/usa envio contínuo de GPS |
